...
import { SteedosQueryOptions, SteedosQueryFilters } from "../src/types/query";
import { SteedosIDType, SteedosObjectType } from "../src/types";
...
// let collection = this.collation(tableName);
// collation.find(mongoFilters, mongoOptions).fetch();
// collation.update(selector, { $set: data }, {validate:false, filter:fasle})
...
export class SteedosMeteorMongoDriver implements SteedosDriver {
	...
	getMongoSortOptions(sort: string): JsonMap {
		...
		if (sort && typeof sort === "string") {
			let arraySort: string[] = sort.split(",").map(n => {
				return n.trim();
			});
