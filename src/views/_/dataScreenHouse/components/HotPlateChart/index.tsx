...
const MaleFemaleRatioChart: React.FC = () => {
	...
	// }, []);
	function GetData() {
		fetch('http://127.0.0.1:1880/wait_out')
			.then(res => res.json())
			.then(value => {
				// console.log('chuku data', value.data)
				const key = value.data.map((value: any, key: any) => ({ ...value, key: key + 1 }));
				setdataSource(key);
			});
	}
							...
							{FakeData.length != 0 ? (
								FakeData.map((value: any, index: number) => (
									<tr key={index} className='w-[100px] text-[14px]'>
										...
										<td className='w-[70px] text-center'>{value.item_code__c}</td>
										<td className='max-w-[110px] text-center text-ellipsis text-nowrap overflow-hidden'>{value.item_name__c}</td>
